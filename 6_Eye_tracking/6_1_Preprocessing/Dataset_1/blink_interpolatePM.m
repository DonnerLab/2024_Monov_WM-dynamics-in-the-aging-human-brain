function [newpupil, newXgaze, newYgaze, newblinksmp, nanIdx, h] = blink_interpolatePM(dat, inFile, plotme)
% interpolates blinks and missing data
% Anne Urai, 2016
% Peter Murphy, 2016 - added adaptive interpolation windows 

% interpolation parameters
padding1         = [-0.150 0.150]; % padding (in s) before/after EL-defined blinks for initial rough interpolation
...
gazethresh       = 10.0;            % z-scored GAZE derivative threshold that samples before/after a bad period must consistently satisfy   (default = 6.0 - gaze tends to be noisier/higher freq than pupil)
...
tmax             = [0.25 0.4];     % max time (s) [start end] points of interpolation window can go from identified bad samples   (default = 0.25 for quick blinkers, 0.5 for slow)
