...
#    }
    ...
    jwg_clusters = {
        'JWG_lr_aIPS1': ['lh.JWDG.lr_aIPS1-lh', 'rh.JWDG.lr_aIPS1-rh',
        ],
        'JWG_lat_aIPS': ['lh.JWG_lat_aIPS-lh', 'rh.JWG_lat_aIPS-rh',
        ],
        'JWG_aIPS1': ['lh.JWG_aIPS1-lh', 'rh.JWG_aIPS1-rh',
        ],
        'JWG_lr_IPS_PCes': ['lh.JWDG.lr_IPS_PCes-lh', 'rh.JWDG.lr_IPS_PCes-rh',
        ],
        'JWG_lat_IPS_PCeS': ['lh.JWG_lat_IPS_PCeS-lh', 'rh.JWG_lat_IPS_PCeS-rh',
        ],
        'JWG_IPS_PCeS': ['lh.JWG_IPS_PCeS-lh', 'rh.JWG_IPS_PCeS-rh',
        ],
        'JWG_lr_M1': ['lh.JWDG.lr_M1-lh', 'rh.JWDG.lr_M1-rh',
        ],
        'JWG_lat_M1': ['lh.JWG_lat_M1-lh', 'rh.JWG_lat_M1-rh',
        ],
        'JWG_M1': ['lh.JWG_M1-lh', 'rh.JWG_M1-rh',
        ],
    }
    ...
    all_clusters.update(glasser_new_clusters)
    areas = [
        item
        for sublist in [all_clusters[k] for k in all_clusters.keys()]
        for item in sublist
    ]
    complete_glasser = get_all_glasser_clusters()
    #complete_glasser.update(jwg_clusters) # adding jwg clusters
    return all_clusters #complete_glasser (= each parcel) #all_clusters (all clusters = as for initial clustering with M1 hand)   #, visual_field_clusters, glasser_clusters, jwg_clusters, glasser_new_clusters
...
def get_all_glasser_clusters_lh():
    # fmt: off
    labels = [
        "V1",	"MST",	"V6",	"V2",	"V3",	"V4",	"V8",	"4"	, "3b",	"FEF",
       	"PEF", "55b",	"V3A",	"RSC",	"POS2",	"V7",	"IPS1",	"FFC",	"V3B",	"LO1",
    ...
    # fmt: on
    return {l:['L_{}_ROI-lh'.format(l)] for l in labels}

def get_all_glasser_clusters_test():
    # fmt: off
    labels = [
        "V1",
    ]
...
def get_all_glasser_clusters_rh():
    # fmt: off
    labels = [
        "V1",	"MST",	"V6",	"V2",	"V3",	"V4",	"V8",	"4"	, "3b",	"FEF",
       	"PEF", "55b",	"V3A",	"RSC",	"POS2",	"V7",	"IPS1",	"FFC",	"V3B",	"LO1",
    ...
    # fmt: on
    return {l:['R_{}_ROI-rh'.format(l)] for l in labels}
