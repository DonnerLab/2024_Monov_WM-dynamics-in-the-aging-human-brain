...
from pymeg import gen_decoding_GM
from pymeg import decoding_GM
from pymeg import decodePipeline as decPi
...
subjects = {'03': [()]}#,,# Subjects
#
# subjects = {'01': [()]}#,,# Test one example
...
mem_demand = {'01': 3, '02': 2, '03': 16, '04': 2, '05': 2, '06': 2,   # SETTINGS FOR VERTEX-AVERAGED DECODING
              '07': 2, '08': 2, '09': 2, '10': 2, '11': 12, '14': 2,
              '15': 2, '16': 5, '17': 2, '18': 2, '19': 3,
              '20': 5, '21': 2, '22': 2, '23': 2, '24': 12, '25': 2,
              '26': 2, '27': 2, '28': 2, '29': 2, '30': 2, '31': 2,
              '32': 12, '33': 2, '35': 2, '36': 2, '37': 5, '38': 2,
              '39': 2, '40': 2, '41': 2, '42': 2, '43': 2, '45': 2,
              '46': 2, '47': 2, '48': 2, '49': 2, '50': 2, '51': 2,
              '52': 2, '53': 5, '54': 2, 'P03': 2}
...
mode = 'whole_cortex' # choose between all_parcels, coarse, or super_roi or complete_glasser or whole_cortex
...
def prep_data(X,ctimes,freqs):
    print('Size:', X.shape)
    X = ((X[:, :, :, ctimes] * X[:, :, :, ctimes].conj()).real)
    oldShape = X.shape
    parts = [X[:, :, idx, :] for idx in np.arange(oldShape[2])]
    freqs = [freqs]*len(parts)
    X = np.concatenate(parts, axis=1)
    freqs = np.concatenate(freqs)

    assert(len(X.shape) == 3)
    assert(X.shape[0] == oldShape[0])
    assert(X.shape[1] == (oldShape[1] * oldShape[2]))
    assert(X.shape[2] == oldShape[3])
...
def decode(
    ...
    for smp in range(len(smpon)):
        ...
        fname = "/home/gmonov/meg_analysis/Decoding/temp_gen_decode_%i_%i_%s/temp_gen_%s_%s_%i_%i_%i.hdf" % (min_freq, max_freq, mode, subject, area, rec, min_freq, max_freq)
        ...
        try:
           ...
           # )
            ...
            tt = [min(times[ctimes]),max(times[ctimes])]
            tt = tuple([tt[0],tt[1]])
            print(str(tt[0]) + ',' +str(tt[1]))
            for target in ["mem_loc"]:
                target_vals = np.int64(np.concatenate(mat["mem_loc"])) #mat[target]
                target_vals = np.vstack(target_vals)
                targets_vals = np.squeeze(np.asarray(target_vals), axis=1)
                targets_vals = np.squeeze(target_vals)
                target_vals = np.squeeze(target_vals, axis=None)
                pred_data = prep_data(tfrdata,ctimes,freq)
                print('Data Size:', pred_data.shape, 'Targets Size:', target_vals.shape)
                prediction_score_matrix = np.empty((len(times),len(times)))
                prediction_score_matrix[:] = np.nan
                prediction_score_matrix = pd.DataFrame(prediction_score_matrix)
                combined_score_data = decPi.crossValEvalDecoder(predictors=pred_data, outcome=target_vals, timePoints=times[ctimes],
                                            numFolds=10, pipeline='default',
                                            trainTimeIdx='all', timeInvWindow=None,
                                            splitVar=None,
                                            evalMode='atAllTimes',
                                            njobs=1)

    print(str(combined_score_data.shape))
    combined_score_data = avOutDfLevel(combined_score_data, "CV_fold")
    print(str(combined_score_data.shape))
    combined_score_data_copy = combined_score_data
    train_time = combined_score_data.index.get_level_values('Time_trained')
    gen_time = combined_score_data.index.get_level_values('Time_evaluated')
    print(combined_score_data.columns)
    from scipy.io import savemat
    savepath = '/mnt/homes/home028/gmonov/meg_analysis/Decoding/temp_gen_decode_%i_%i_%s/temp_gen_'  % (min_freq, max_freq, mode)
    mdic = {'train_time': train_time.values,
            'gen_time': gen_time.values,
            'corrs': combined_score_data_copy.values,
            'times': times[ctimes]}
    savemat(savepath + subject + '_' + area + '_' + str(rec) + '_' + str(min_freq) + '_' + str(max_freq) + '.mat', mdic)
