...
class Decoder(object):
    ...
    def classify(self, data, time, freq, trial, roi,
        ...
        for idx_t, tp in enumerate(time):
            ...
            scaler.fit(X_new[~idnan, :])
            X_new = scaler.transform(X_new[~idnan, :])
            pca = PCA(0.95)
            pca.fit(X_new[~idnan, :])
            print(pca.n_components_)
            s["n_comps_95"] = pca.n_components_
            pca = PCA(0.90)
            pca.fit(X_new[~idnan, :])
            print(pca.n_components_)
            s["n_comps_90"] = pca.n_components_
            pca = PCA(0.85)
            pca.fit(X_new[~idnan, :])
            print(pca.n_components_)
            s["n_comps_85"] = pca.n_components_
            pca = PCA(0.80)
            pca.fit(X_new[~idnan, :])
            print(pca.n_components_)
            s["n_comps_80"] = pca.n_components_
            scores.append(s)
...
def categorize(clf, target, data, njobs=1):
    ...
    score = cross_validate(
        clf, data, target, cv=10, scoring='roc_auc', return_train_score=False, n_jobs=njobs
    )
    del score["fit_time"]
