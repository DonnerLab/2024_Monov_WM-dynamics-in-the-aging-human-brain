...
#SBATCH --export=NONE
#SBATCH --mem={memory}'''.format(walltime=walltime,
               nodes=nodes,
               memory=memory,
               tasks=tasks,
               name=name)
...
#SBATCH --mail-type=ALL'''.format(email=email)
    sbatch_directives += '''
#SBATCH --error={logdir}/slurm_%j.out
#SBATCH --output={logdir}/slurm_%j.err'''.format(logdir=logdir)
    if env is not None:
        sbatch_directives += '''
source ~/miniconda3/etc/profile.d/conda.sh
conda activate {env}'''.format(env=env)
    ...
    for arg in args:
        if check_store:
            try:
                ...
                pass
        script = 'python ' + to_script(func, tmpdir, *arg)
        pid = script
        if verbose:
            print(arg, '->', script)
